...
int main(int argc, char* argv[]) {
  ...
  uint64_t *exp    = malloc(size * sizeof(uint64_t));
...
void factor(uint64_t n, uint64_t *primes, uint64_t *exp, uint32_t *size, uint32_t *factors) {
  uint32_t i,e,found,toggle,bound=(uint32_t)sqrt(n)+1;
...
void expand(uint64_t ** const arr1, uint64_t ** const arr2, uint32_t * size) {
  *size *= 2;
  *arr1 = realloc(*arr1, *size * sizeof(uint64_t));
  *arr2 = realloc(*arr2, *size * sizeof(uint64_t));
}
