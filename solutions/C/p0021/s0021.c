...
int main(int argc, char* argv[]) {
  ...
  uint32_t i,total,pCount,fCount;
  uint32_t *memo=NULL,*primes=NULL,**factors=NULL;
  ...
  for(total=0,i=2; i<limit; ++i) {
    factor(i,&factors,&fCount,primes,pCount);
    memo[i] = divisorSum(factors,fCount) - i;
    if(memo[i] < i && memo[memo[i]] == i)
      total += memo[i] + memo[memo[i]];
  }
  printf("%u\n",total);
...
void print_help(const char * const str) {
  ...
  printf("  Numbers a & b are amicable numbers iff d(a) = b and d(b) = a and a != b\n");
