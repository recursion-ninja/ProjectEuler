    ...
    show fs@(FactorSet im) =
        let numStr key val = [ (show key, showPower $ val `div` 2) ]
            ...
            showDigit = \case
                ...
                '9' -> '⁹'
                _   -> '⁰'
        in  unwords [ squaredStr fs, "=", factorsStr im, "=", productStr fs ]
...
solution :: ConcealedSquare -> Maybe FactorSet
solution =
    let solutionStream :: ConcealedSquare -> [FactorSet]
        solutionStream = solveConcealedSquare
        getFirstFound  = getFirst . foldMap (First . Just)
    in  getFirstFound . solutionStream 
